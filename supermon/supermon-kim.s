...
; source.
; 
; SUPERMON+ 64 was one of the most powerful and popular machine
; language monitors for the Commodore 64.  It evolved out of monitors
; for older Commodore machines such as the the PET and possibly even
; the KIM-1 itself.  I've taken the source and ported it back to the
; KIM once more.
;
; Some features of the original are particularly suited to the
; Commodore full screen editor and line buffered input, and don't work
; quite as well with a serial console.  Even so, the monitor is much
; more useful than the primitive built-in KIM monitor.
;
; A few of the notable changes to adapt to the KIM-1:

; * "L"oad calls the KIM monitor LOAD routine to load a paper tape
;   file; that routine is hardcoded to jump back to the KIM monitor on
;   completion, so leaves you back in the KIM monitor
; * "S"ave, and "V"erify are left unimplemented
; * The loader installs the monitor as both the IRQ and NMI handler,
;   so both the "ST"op button and a BRK trigger SUPERMON
...
; -----------------------------------------------------------------------------
; setup ca65 for the supermon source dialect

.feature labels_without_colons, loose_char_term, pc_assignment, leading_dot_in_identifiers
...
; -----------------------------------------------------------------------------
; kernal variables
NMIVEC  = $17FA             ; NMI vector (triggered by ST key on KIM)
BKVEC   = $17FE             ; BRK instruction vector (shared with IRQ)
...
DISPMEM_BPL      .FILL 1    ; number of bytes per line to display in DISPMEM
DISPMEM_BPL_LOG2 .FILL 1    ; log 2 of DISPMEM_BPL

; -----------------------------------------------------------------------------
; kernal entry points (KIM)
LOAD    = $1CE7             ; load from paper tape
GETCH   = $1E5A             ; input a character (like CHRIN)
OUTCH   = $1EA0             ; output a character (like CHROUT)
...
SUPER   LDY #MSG4-MSGBAS    ; display banner
        JSR SNDMSG
        LDA SUPAD
        LDx SUPAD+1
        ...
        LDA LINKAD          ; set BRK and NMI vectors
        STA NMIVEC
        STA BKVEC
        LDA LINKAD+1
        STA NMIVEC+1
...
; -----------------------------------------------------------------------------
; BRK handler
BREAK   STA ACC             ; store A,X,Y directly
        STX XR
        STY YR
        LDX #$02            ; pull registers off the stack
BSTACK  PLA                 ; order: SR,PCL,PCH
...
SMOVE   JSR INPUT           ; CHRIN kernal call to input a character
        ...
        INX                 ; go back
        JMP SMOVE
NBS     STA INBUFF,X        ; store in input buffer
        INX 
...
; -----------------------------------------------------------------------------
; dispatch command
S2      CPX #$10            ; last 4 commands are base conversions
...
; -----------------------------------------------------------------------------
; exit monitor [X]
EXIT    JMP $1C00           ; jump to kim monitor
...
SNDCLR  JSR SNDMSG
SNCLR   RTS                 ; KIM CHANGE: no screen editor, don't worry about clearing to eol
...
MSG4    .BYTE $0D, $0D, "SUPERMON KIM " ; SUPERMON KIM banner
        .BYTE "@ ", '$'+$80
...
KADDR   .WORD ASSEM-1,COMPAR-1,DISASS-1,FILL-1
        ...
        .WORD ASSEM-1,ALTM-1,ALTR-1,KIM_LD-1,KIM_SV-1
...
; -----------------------------------------------------------------------------
; KIM implementations of load, save and the KERNAL routines used by SUPERMON
K_SAVED_Y: .byte 0

; -----------------------------------------------------------------------------
; KIM LD - load from paper tape - note this jumps back to the KIM monitor entry
KIM_LD  JMP LOAD

KIM_SV  JMP ERROR

; -----------------------------------------------------------------------------
; KIM STOP - clears the Z bit to indicate the stop key is not pressed;
;            the KIM stop ("ST") key triggers an NMI so this is not
;            needed
STOP    LDA #$FF
        RTS

; -----------------------------------------------------------------------------
; KIM CHROUT - calls OUTCH but also maintains the Y register
CHROUT  STY K_SAVED_Y
        JSR OUTCH
        LDY K_SAVED_Y
        RTS

; -----------------------------------------------------------------------------
; KIM INPUT - calls GETCH but also maintains the Y register and maps lower case to upper case
INPUT   STY K_SAVED_Y
        JSR GETCH
        CMP #"a"
        BCC NOCASECONV
